  ...
  rateLimit: {
    ...
    general: {
      ...
      message: {
        ...
        message: 'ÙŠØ±Ø¬Ù‰ Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø© Ù…Ø±Ø© Ø£Ø®Ø±Ù‰ Ø¨Ø¹Ø¯ ÙØªØ±Ø©',
    ...
    login: {
      ...
      message: {
        ...
        message: 'ÙŠØ±Ø¬Ù‰ Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø© Ù…Ø±Ø© Ø£Ø®Ø±Ù‰ Ø¨Ø¹Ø¯ 15 Ø¯Ù‚ÙŠÙ‚Ø©',
        retryAfter: '15 Ø¯Ù‚ÙŠÙ‚Ø©',
      },
      standardHeaders: true,
      legacyHeaders: false,
    ...
    document: {
      ...
      message: {
        error: 'ØªÙ… ØªØ¬Ø§ÙˆØ² Ø­Ø¯ Ø±ÙØ¹ Ø§Ù„Ù…Ù„ÙØ§Øª',
        message: 'ÙŠØ±Ø¬Ù‰ Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø© Ù…Ø±Ø© Ø£Ø®Ø±Ù‰ Ø¨Ø¹Ø¯ 5 Ø¯Ù‚Ø§Ø¦Ù‚',
    ...
    search: {
      ...
      message: {
        error: 'ØªÙ… ØªØ¬Ø§ÙˆØ² Ø­Ø¯ Ø¹Ù…Ù„ÙŠØ§Øª Ø§Ù„Ø¨Ø­Ø«',
        message: 'ÙŠØ±Ø¬Ù‰ Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø© Ù…Ø±Ø© Ø£Ø®Ø±Ù‰ Ø¨Ø¹Ø¯ Ø¯Ù‚ÙŠÙ‚Ø©',
...
export const createEnhancedRateLimiter = (type: keyof typeof SECURITY_CONFIG.rateLimit) => {
  ...
  const ipLimiter = rateLimit({
    ...
    // Use default key generator to handle IPv6 correctly
  ...
  const userLimiter = rateLimit({
    ...
    skipSuccessfulRequests: config.skipSuccessfulRequests,
    skipFailedRequests: config.skipFailedRequests,
    // Default key generator; skip unauthenticated below
    skip: (req: Request) => {
  ...
  const burstLimiter =
    config.burstWindowMs && config.burstMax
      ? rateLimit({
        ...
        standardHeaders: config.standardHeaders,
        legacyHeaders: config.legacyHeaders,
        // Use default key generator to properly support IPv6
        handler: (req: Request, res: Response) => {
          const logData = {
            ip: req.ip,
            url: req.url,
...
export const requestValidation = (req: Request, res: Response, next: NextFunction) => {
  ...
  if ((req.method === 'POST' || req.method === 'PUT') && req.path.startsWith('/api/')) {
    ...
    if (!contentType || !contentType.includes('application/json')) {
      return res.status(400).json({
        error: 'Ù†ÙˆØ¹ Ø§Ù„Ù…Ø­ØªÙˆÙ‰ ØºÙŠØ± ØµØ§Ù„Ø­',
        message: 'ÙŠØ¬Ø¨ Ø£Ù† ÙŠÙƒÙˆÙ† Ø§Ù„Ù…Ø­ØªÙˆÙ‰ Ù…Ù† Ù†ÙˆØ¹ JSON',
  ...
  if (contentLength > maxSize) {
    return res.status(413).json({
      error: 'Ø­Ø¬Ù… Ø§Ù„Ø·Ù„Ø¨ ÙƒØ¨ÙŠØ± Ø¬Ø¯Ø§Ù‹',
      message: 'Ø§Ù„Ø­Ø¯ Ø§Ù„Ø£Ù‚ØµÙ‰ Ù„Ø­Ø¬Ù… Ø§Ù„Ø·Ù„Ø¨ Ù‡Ùˆ 10 Ù…ÙŠØ¬Ø§Ø¨Ø§ÙŠØª',
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!allowedIPs.includes(clientIP) && !allowedIPs.includes('*')) {
      ...
      return res.status(403).json({
        ...
        message: 'Ø¹Ù†ÙˆØ§Ù† IP ØºÙŠØ± Ù…Ø³Ù…ÙˆØ­ Ø¨Ù‡',
        code: 'IP_NOT_ALLOWED',
        timestamp: new Date().toISOString(),
      });
    }
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!apiKey || !apiKeys[apiKey]) {
      return res.status(401).json({
        error: 'Ù…ÙØªØ§Ø­ API ØºÙŠØ± ØµØ§Ù„Ø­',
