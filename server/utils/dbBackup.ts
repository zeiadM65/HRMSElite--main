...
export class DatabaseBackupManager {
   ...
   * Encrypt file
   */
  private encryptFile(inputPath: string, outputPath: string): { iv: string; tag: string } {
   ...
   * Decrypt file
   */
  private decryptFile(inputPath: string, outputPath: string, iv: string, tag: string): void {
    const key = crypto.scryptSync(this.config.encryptionKey, 'salt', 32);
  ...
  public async createBackup(dbPath?: string, type: 'manual' | 'scheduled' = 'manual'): Promise<BackupMetadata> {
    try {
      ...
      if (this.config.encryptionKey) {
        ...
        ({ iv, tag } = this.encryptFile(finalBackupPath, encryptedPath));
