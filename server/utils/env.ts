...
// Load environment variables from .env files early (development convenience)
try {
  ...
  for (const file of candidates) {
    if ((fs as any).existsSync(file)) {
      (dotenv as any).config({ path: file });
    }
  }
} catch {
  // ignore dotenv loading failures; rely on process.env in prod
}

// استبدال القيم الافتراضية للأسرار في ملفات الإعدادات
const DEFAULT_DB_SECRET = 'actual_production_secret';
const DB_SECRET = process.env.DB_SECRET ?? DEFAULT_DB_SECRET;

const forbidDefaults = (name: string, val: string) => {
  if (!val || val === DEFAULT_DB_SECRET) throw new Error(`Env ${name} must be set (not placeholder).`);
};

// Legacy JWT_SECRET is optional when ACCESS_JWT_SECRET is provided
if (process.env.JWT_SECRET) {
  forbidDefaults('JWT_SECRET', process.env.JWT_SECRET!);
}
// DB_ENCRYPTION_KEY is required (validated below as well)
forbidDefaults('DB_ENCRYPTION_KEY', process.env.DB_ENCRYPTION_KEY!);
// Only enforce DB_SECRET if explicitly provided
